...
team_matches_home = db.Table(
    "team_matches_home",
    db.Column("team_id", db.Integer, db.ForeignKey("team.id")),
    db.Column("match_id", db.Integer, db.ForeignKey("match.id"), primary_key=True),
)
team_matches_away = db.Table(
    "team_matches_away",
    db.Column("team_id", db.Integer, db.ForeignKey("team.id")),
    db.Column("match_id", db.Integer, db.ForeignKey("match.id"), primary_key=True),
)

model_seasons = db.Table(
    "model_seasons",
...
class Model(Output, db.Model):
    ...
    seasons = db.relationship(
        "Season", secondary=model_seasons, back_populates="models"
    )
    ...
    features = db.relationship(
        "Feature", secondary=model_features, back_populates="models"
    )
    ...
    def retrieve_feature_values(self, matches=None):
        ...
        if not matches:
            matches = filter(
                lambda match: match.status == "FINISHED",
                list(chain(*[season.matches for season in self.seasons])),
            )
    ...
    def retrieve_targets(self) -> List[str] | None:
        ...
        for match in chain(*[season.matches for season in self.seasons]):
            ...
            if (
                match.matchday > self.number_of_last_games
    ...
    def train(self):
        ...
        if classifier:
            self.classifier = pickle.dumps(
                classifier.fit(np.array(feature_values), targets)
            )
    ...
    def predict(self, matches) -> str | None:
        ...
        if self.classifier:
            return pickle.loads(self.classifier).predict(
                np.array(self.retrieve_feature_values(matches))
            )
...
class Feature(Output, db.Model):
    ...
    models = db.relationship(
        "Model", secondary=model_features, back_populates="features"
    )
    ...
    def retrieve_values(self, model, matches):
        ...
        if self.name == "fulltime_goals" or self.name == "halftime_goals":
            for match in matches:
                ...
                if (
                    match.matchday > model.number_of_last_games
                    and match.matchday <= match.season.current_matchday + 1
                ):
                    matchday = match.matchday
                    # get matches before the match according to number of matches to be used for the model
                    home_matches_home_team = filter(
                        lambda match_: match_.status == "FINISHED"
                        and match_.season == match.season
                        and (matchday - model.number_of_last_games)
                        <= match_.matchday
                        < matchday,
                        match.home_team.matches_home,
                    )
                    away_matches_home_team = filter(
                        lambda match_: match_.status == "FINISHED"
                        and match_.season == match.season
                        and (matchday - model.number_of_last_games)
                        <= match_.matchday
                        < matchday,
                        match.home_team.matches_away,
                    )
                    home_matches_away_team = filter(
                        lambda match_: match_.status == "FINISHED"
                        and match_.season == match.season
                        and (matchday - model.number_of_last_games)
                        <= match_.matchday
                        < matchday,
                        match.away_team.matches_home,
                    )
                    away_matches_away_team = filter(
                        lambda match_: match_.status == "FINISHED"
                        and match_.season == match.season
                        and (matchday - model.number_of_last_games)
                        <= match_.matchday
                        < matchday,
                        match.away_team.matches_away,
                    )
                    ...
                    if self.name == "fulltime_goals":
                        for match in home_matches_home_team:
                            match_features[
                                model.number_of_last_games
                                - 1
                                + (matchday - match.matchday - 1)
                            ] = match.score.fulltime_goals_home
                        for match in away_matches_home_team:
                            match_features[
                                model.number_of_last_games
                                - 1
                                + (matchday - match.matchday - 1)
                            ] = match.score.fulltime_goals_away
                        for match in home_matches_away_team:
                            match_features[
                                2 * model.number_of_last_games
                                - 1
                                + (matchday - match.matchday - 1)
                            ] = match.score.fulltime_goals_home
                        ...
                        for match in home_matches_home_team:
                            match_features[
                                ...
                                - 1
                                + (matchday - match.matchday - 1)
                            ] = match.score.halftime_goals_home
                        for match in away_matches_home_team:
                            match_features[
                                model.number_of_last_games
                                - 1
                                + (matchday - match.matchday - 1)
                            ] = match.score.halftime_goals_away
                        for match in home_matches_away_team:
                            match_features[
                                2 * model.number_of_last_games
                                - 1
...
class Match(Output, db.Model):
    ...
    away_team = db.relationship(
        "Team",
        secondary=team_matches_away,
        back_populates="matches_away",
        uselist=False,
    )
    home_team = db.relationship(
        "Team",
        secondary=team_matches_home,
...
class Team(Output, db.Model):
    ...
    matches_away = db.relationship(
        "Match", secondary=team_matches_away, back_populates="away_team"
    )
    matches_home = db.relationship(
        "Match", secondary=team_matches_home, back_populates="home_team"
    )
