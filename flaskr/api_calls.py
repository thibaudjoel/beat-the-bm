...
def get_matches(date_from, date_to, league_ids) -> None:
    ...
    while end < date_to:
        response = requests.get(url, params={'competitions': league_ids, 'dateFrom': start.strftime("%Y-%m-%d"), 'dateTo': end.strftime("%Y-%m-%d")}, headers={'X-Auth-Token': f"{os.environ.get('FD_API_KEY')}"})
        ...
        for match in dict.get("matches"):
            ...
            if not season:
                ...
                season.start_date = datetime.strptime(match.get('season').get('startDate'), '%Y-%m-%d')
                season.end_date = datetime.strptime(match.get('season').get('endDate'), '%Y-%m-%d')
            ...
            new_match.match_date_time = datetime.strptime(match.get('utcDate'), '%Y-%m-%dT%H:%M:%SZ')
...
def get_seasons(league_id: int) -> None:
    ...
    response = requests.get(url, headers={'X-Auth-Token': f"{os.environ.get('FD_API_KEY')}"})
    dict = response.json()
    ...
    for season in dict.get("seasons"):
        ...
        if not query_season_on_id(season.get('id')):
            ...
            new_season.start_date = datetime.strptime(season.get('startDate'), '%Y-%m-%d')
            ...
            if start_year == 2022 or start_year == 2023:
                ...
                response = requests.get(url, params={'season': start_year}, headers={'X-Auth-Token': f"{os.environ.get('FD_API_KEY')}"})
                dict = response.json()
...
def get_matches_of_season(league_id: str, start_year: str, match_day: int) -> None:
    ...
    response = requests.get(url, params={'season': start_year, 'matchday': match_day}, headers={'X-Auth-Token': f"{os.environ.get('FD_API_KEY')}"})
    dict = response.json()
